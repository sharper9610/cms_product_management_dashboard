...
document.addEventListener('DOMContentLoaded', function (e) {
  ...
  if (userPermissions.includes('settings.prompt.management')) {
    buttons.push({
      text: '<i class="icon-base ri ri-add-line icon-sm me-0 me-sm-2 d-sm-none d-inline-block"></i><span class="d-inline-block">Add Prompt</span>',
  ...
  if (dt_user_table) {
    const dt_user = new DataTable(dt_user_table, {
      serverSide: true,
      ajax: {
        url: baseUrl+'settings/prompt-management',
        type: 'GET',
        beforeSend: function () {
      ...
      columnDefs: [
...

        {
          ...
          responsivePriority: 4,
          render: function(data, type, full, meta) {
            var status = full['is_active']; // 0 or 1
  ...
  $('.datatables-users tbody').on('click', '.delete-record', function () {
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        $.ajax({
          url: baseUrl + 'settings/prompt-management/' + userId,
          type: "DELETE",
...
// Edit User Form Validation
  ...
  fvEdit.on('core.form.valid', function () {
    ...
    $.ajax({
      url: `${baseUrl}settings/prompt-management/${promptId}`,
      type: 'PUT',
      data: $form.serialize(),
      headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content'),
  ...
  $.ajax({
    ...
    success: function (response) {
      if (response.success && offcanvasElement.length) {
        ...
        // Fill form
        $("#offcanvasEditUser .edit-prompt-id").val(response.prompt.id);
        $("#offcanvasEditUser .edit-name").val(response.prompt.name);
        $("#offcanvasEditUser .edit-template").val(response.prompt.template);
        $("#offcanvasEditUser .edit-template_pt").val(response.prompt.template_pt);
        $("#offcanvasEditUser .edit-template_es").val(response.prompt.template_es);

        $("#offcanvasEditUser .edit-template_gift_card").val(response.prompt.template_gift_card);
        $("#offcanvasEditUser .edit-template_gift_card_pt").val(response.prompt.template_gift_card_pt);
        $("#offcanvasEditUser .edit-template_gift_card_es").val(response.prompt.template_gift_card_es);

        $("#offcanvasEditUser .edit-description").val(response.prompt.description);
        $("#offcanvasEditUser .edit-status").val(response.prompt.status);
