...
document.addEventListener('DOMContentLoaded', function (e) {
  ...
  if (dt_user_table) {
    const dt_user = new DataTable(dt_user_table, {
      serverSide: true,
      ajax: {
        url: baseUrl+'settings/dlc-product-id-prompt-management',
        type: 'GET',
        beforeSend: function () {
      ...
      columnDefs: [
        ...
        {
          targets: 1, // adjust the column index

          render: function (data, type, full, meta) {
            var template = full['template'] || ''; // safe fallback if null
            ...
            var row_output =
              ...
              '<span class="fw-medium" style="white-space: pre-wrap;">' + template + '</span>' +
              '</div>';
        ...
        {
          targets: 2, // adjust this to your column index
  ...
  $('.datatables-users tbody').on('click', '.delete-record', function () {
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        $.ajax({
          url: baseUrl + 'settings/dlc-product-id-prompt-management/' + userId,
          type: "DELETE",
...
// Edit User Form Validation
  ...
  fvEdit.on('core.form.valid', function () {
    ...
    $.ajax({
      url: `${baseUrl}settings/dlc-product-id-prompt-management/${promptId}`,
      type: 'PUT',
      data: $form.serialize(),
      headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content'),
