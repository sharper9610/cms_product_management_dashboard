...
class ProcessStorefrontOrderRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'items.*.source_sku'                => 'nullable|integer',
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            // ── Allowlist (temporary) ─────────────────────────────────────────
            $allowedParentSkus = [363750];
            $allowedSourceSkus = [36375];

            foreach ($items as $index => $item) {
                $parentSku = (int) ($item['product_id'] ?? 0);
                $sourceSku = isset($item['source_sku']) ? (int) $item['source_sku'] : null;

                if (!in_array($parentSku, $allowedParentSkus, true)) {
                    $validator->errors()->add(
                        "items.{$index}.product_id",
                        "Product ID {$parentSku} is not currently supported on the storefront."
                    );
                }

                if ($sourceSku !== null && !in_array($sourceSku, $allowedSourceSkus, true)) {
                    $validator->errors()->add(
                        "items.{$index}.source_sku",
                        "Source SKU {$sourceSku} is not currently supported on the storefront."
                    );
                }
            }
        });
