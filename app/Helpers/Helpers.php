...
class Helpers
{
    ...
    {
        return array_keys(config('currency'));
    }

    public static function getAllowedCountries()
    {
        return Arr::flatten(config('currency'));
    }

    public static function logOrShow(string $msg, string $level='debug')
    {
    ...
    public static function validateLengthOffset(Request $request)
    {
        $validator = Validator::make(
            ...
            [
                'length.required' => 'The length query parameter is required.',
                'length.integer' => 'The length must be an integer.',
                'length.min' => 'The length must be at least 1.',
                'length.max' => 'The length may not be greater than 1000.',
            ]
        );
